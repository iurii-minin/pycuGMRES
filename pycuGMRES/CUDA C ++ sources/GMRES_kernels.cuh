...
//at 0x00000228 in extend_by_zeros_kernel(bool*, float2*, float2*, unsigned int)
...
__global__ void extend_by_zeros_kernel(bool *dev_mask, cuComplex *dev_usual,
		cuComplex *dev_extended, const unsigned int N) //For usual matvec (mask is present)
		{
...
//	{	
	...
	if ((i < N) && (j < N) && (dev_mask[index])) {
		...
		dev_extended[index + Ni - i] = current;
	} else {
		if ((i < 2 * N - 1) && (j < 2 * N - 1)) {
...
__global__ void extend_by_zeros_kernel(cuComplex *dev_usual, //For Gradient matvec (mask is absent)
		cuComplex *dev_extended, const unsigned int N) {
	unsigned int i = Q * blockIdx.x + threadIdx.x;
	unsigned int j = Q * blockIdx.y + threadIdx.y;
//	unsigned int size_limit = (N << 1) - 1;
	cuComplex current;
...
__global__ void MatMul_ElemWise_kernel(cuComplex *dev_bttb_sur,
		cuComplex *dev_vec2D, const unsigned int N) {
	unsigned int i = Q * blockIdx.x + threadIdx.x;
	unsigned int j = Q * blockIdx.y + threadIdx.y;
	unsigned int size_limit = (N << 1) - 1;
	...
	if ((i < size_limit) && (j < size_limit)) {
		...
		dev_vec2D[index].x = (curr_bttb.x * curr_out_mul.x
				- curr_out_mul.y * curr_bttb.y);
		dev_vec2D[index].y = (curr_out_mul.x * curr_bttb.y
				+ curr_out_mul.y * curr_bttb.x);
	}
}
...
__global__ void _2D_to_1D_compared_kernel(cuComplex *dev_input_mul, //For GMRES to compute LES for Helmholtz equation (mask and target index are absent)
		cuComplex *dev_2D_in, cuComplex *dev_residual, const unsigned int N) {
	unsigned int i = Q * blockIdx.x + threadIdx.x;
	unsigned int j = Q * blockIdx.y + threadIdx.y;
	unsigned int size_limit = (N << 1) - 1;
	...
	if ((i < size_limit) && (j < size_limit)) {
		...
		Input_Field.x += current_2D.x / ((N << 1) - 1) / ((N << 1) - 1)
				- arg_old.x;
		Input_Field.y += current_2D.y / ((N << 1) - 1) / ((N << 1) - 1)
				- arg_old.y;
...
__global__ void _2D_to_1D_compared_kernel(
		bool *dev_mask, //For Gradient of Helmholtz equation-based LES GMRES (mask and target index are present) //#CHANGED
		cuComplex *dev_input_mul, cuComplex *dev_2D_in, cuComplex *dev_residual,
		const unsigned int h_index_of_max, const unsigned int N) {
	unsigned int i = Q * blockIdx.x + threadIdx.x;
	unsigned int j = Q * blockIdx.y + threadIdx.y;
	unsigned int size_limit = (N << 1) - 1;
	...
	if ((i < size_limit) && (j < size_limit)) {
		...
		current.x =
				(h_index_of_max == _1D_index) ? 1.f - arg_old.x : -arg_old.x;//"=" operation
		current.y = -arg_old.y;
...
__global__ void residual_normalization_kernel(cuComplex *dev_residual_vec,
		float *dev_norm_res_vec, cuComplex *dev_orthogonal_basis) {
...
__global__ void _2D_to_1D_kernel(cuComplex *dev_input_mul, cuComplex *dev_2D_in,
		cuComplex *dev_1D_out, const unsigned int N) {
	unsigned int i = Q * blockIdx.x + threadIdx.x;
	unsigned int j = Q * blockIdx.y + threadIdx.y;
	unsigned int Ni = N * i;
	unsigned int _1D_index = Ni + j;
...
__global__ void _2D_to_1D_kernel(
		bool *dev_mask, //For gradient computations
		cuComplex *dev_input_mul, cuComplex *dev_2D_in, cuComplex *dev_1D_out,
...
__global__ void weight_subtract_kernel(cuComplex *dev_weight,
		cuComplex *dev_Hjk, cuComplex *dev_vj) {
...
__global__ void get_complex_divided(const float *dev_Hjk_norm_part,
		cuComplex *dev_Hj, float *dev_divided) {
	switch (blockIdx.x) {
	case 0: {
...
__global__ void set_first_Jtotal_kernel(cuComplex *dev_Jtotal, cuComplex *Htemp,
		const unsigned int maxiter, const unsigned int characteristic_size) {
	switch (blockIdx.x) {
	case 0: {
		float denominator = sqrt(
				pow((float) Htemp->x, 2.f) + pow((float) Htemp[maxiter].x, 2.f)
						+ pow((float) Htemp->y, 2.f)
						+ pow((float) Htemp[maxiter].y, 2.f));
	...
	default: {
		...
		case 0: {
			float denominator = sqrt(
					pow((float) Htemp->x, 2.f)
							+ pow((float) Htemp[maxiter].x, 2.f)
							+ pow((float) Htemp->y, 2.f)
							+ pow((float) Htemp[maxiter].y, 2.f));
		...
		default: {
			if (blockIdx.x % 2) {
				...
				dev_Jtotal[index].x =
						index % (characteristic_size + 1) ? 0.f : 1.f;
			} else {
...
__global__ void set_4_Givens_rotation_matrix_elements_kernel(
		cuComplex *dev_Htemp, const unsigned int characteristic_size,
		cuComplex *dev_Givens_rotation_0, cuComplex *dev_Givens_rotation_1,
		cuComplex *dev_Givens_rotation_2, cuComplex *dev_Givens_rotation_3,
	...
	float denominator = sqrt(
			pow((float) dev_Htemp[index_H_0].x, 2.f)
					+ pow((float) dev_Htemp[index_H_1].x, 2.f)
					+ pow((float) dev_Htemp[index_H_0].y, 2.f)
					+ pow((float) dev_Htemp[index_H_1].y, 2.f));
...
__global__ void set_cc_kernel(cuComplex *dev_cc, cuComplex *dev_Jtotal,
		float *dev_old_norm_res_vec, const unsigned int characteristic_size) {
...
__global__ void get_new_solution_kernel(cuComplex *dev_cc, cuComplex *dev_HH) {
	float dominant = pow((float) (dev_HH->x), 2.f)
			+ pow((float) (dev_HH->y), 2.f);
	cuComplex current;
...
__global__ void get_solution_kernel(cuComplex *dev_solution, cuComplex *dev_cc,
		cuComplex *dev_orthogonal_basis) {
	unsigned int index = blockIdx.x * blockDim.x + threadIdx.x;
	cuComplex current = dev_orthogonal_basis[index];
	atomicAdd((float *) &(dev_solution[index].x),
			current.x * dev_cc->x - current.y * dev_cc->y);
	atomicAdd((float *) &(dev_solution[index].y),
			current.x * dev_cc->y + current.y * dev_cc->x);
}

__global__ void add_kernel(cuComplex *dev_solution, cuComplex *dev_add_x,
		cuComplex *dev_mul) {
	unsigned int index = blockIdx.x * blockDim.x + threadIdx.x;
	atomicAdd((float *) &(dev_solution[index].x),
			dev_mul->x * dev_add_x[index].x - dev_mul->y * dev_add_x[index].y);
	atomicAdd((float *) &(dev_solution[index].y),
			dev_mul->y * dev_add_x[index].x + dev_mul->x * dev_add_x[index].y);
}

__global__ void init_x0_kernel(cuComplex *dev_input, const unsigned int N) {
	unsigned int i = Q * blockIdx.x + threadIdx.x;
	unsigned int j = Q * blockIdx.y + threadIdx.y;
	cuComplex Input_Field;
