void cuSolve_LES(cuComplex *dev_A, cuComplex *dev_B, const int m,
		cusolverDnHandle_t cusolverH, cuComplex *dev_work, int *dev_Ipiv,
		int *dev_info, timespec *h_computation_times, unsigned int *clock_i_p);

void show_norm(const char *description, cublasHandle_t handle,
		const cuComplex *dev_array, const unsigned int size_array);

void show_norm_F(const char *description, cublasHandle_t handle,
		const float *dev_array, const unsigned int size_array);
		
void Fast_GMRES_with_CUDA(const cuComplex *dev_gamma_array,
		const bool *dev_mask, cuComplex *dev_solution,
		float *dev_actual_residual, unsigned int *GMRES_n, cufftHandle plan,
		cublasHandle_t *handle_p, const float tolerance,
		const bool for_gradient, const unsigned int h_index_of_max,
		bool *h_res_vs_tol_p, unsigned int maxiter,
		cusolverDnHandle_t cusolverH, timespec *h_computation_times,
		const unsigned int N) {
	unsigned int clock_i = 0;
	
//	timespec ts_example;

	// auto h_computation_times_a = std::array<unsigned long long, 1000000>{} ;
//	auto init_chr = std::chrono::high_resolution_clock::now();
//	void* h_computation_times_a = (void*)malloc(sizeof(init_chr) * 1000000);

	// std::chrono::time_point<std::chrono::high_resolution_clock>* h_computation_times_a = (std::chrono::time_point<std::chrono::high_resolution_clock> *)malloc(sizeof(std::chrono::time_point<std::chrono::high_resolution_clock>) * 100000);


	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
	...
	// cudacall(
	//		cudaMalloc((void**) dev_actual_residual,
	//				maxiter_plus_1 * sizeof(float)));

	cudacall(
			cudaMalloc((void**) &dev_orthogonal_basis,
					((maxiter + 6) * N * N + 8 * N + 1) * sizeof(cuComplex))); //- 8 * N + 1
	...
	dev_Givens_rotation_3 = dev_Givens_rotation
			+ maxiter_plus_1 * maxiter_plus_1 - 1;
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
	cuComplex* h_vec_residual = (cuComplex*)malloc( N * N * sizeof(cuComplex));
	cudacall(
		cudaMemcpyAsync(h_vec_residual, dev_solution,
			N * N * sizeof(cuComplex), cudaMemcpyDeviceToHost));
/*
	float a = {};
	for (int ij = 0; ij < N * N; ij++)
	{
		a += abs(h_vec_residual[ij].x) + abs(h_vec_residual[ij].y);
	}
	fprintf(stderr, "abs_norm = %1.36f\n", a);
*/
//========================================= BEGIN: get_residual_vector =======================================================
	if (for_gradient) {
		G_x_fft_matvec((cuComplex *) dev_gamma_array,
				(cuComplex *) dev_solution, (cuComplex *) dev_extended,
				(cufftHandle) plan, N);
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
		...
		_2D_to_1D_compared_kernel<<<blocks, threads>>>((bool *) dev_mask,
				(cuComplex *) dev_solution, (cuComplex*) dev_extended,
				(cuComplex*) dev_vec_resudual, h_index_of_max, N);
	} else {
		G_x_fft_matvec((cuComplex *) dev_gamma_array, (bool *) dev_mask,
				(cuComplex *) dev_solution, (cuComplex *) dev_extended,
				(cufftHandle) plan, N);
...
//========================================== END: get_residual_vector =========================================================
	cublascall(
			cublasScnrm2((cublasHandle_t) *handle_p, N * N,
	...
	// cuComplex* h_vec_residual = (cuComplex*)malloc( N * N * sizeof(cuComplex));
	// cudacall(
	// 	cudaMemcpyAsync(h_vec_residual, dev_vec_resudual,
	// 		N * N * sizeof(cuComplex), cudaMemcpyDeviceToHost));
	// float a = {};
	// for (int ij = 0; ij < N * N; ij++)
	// {
...
//============================================= Begin: Condition to iterate ==========================================================
	cudacall(
			cudaMemcpyAsync(&h_residual_initial, dev_actual_residual,
...
//============================================BEGIN:residual_normalization_kernel=======================================================
	residual_normalization_kernel<<<gridsize, blocksize>>>(
			(cuComplex *) dev_vec_resudual, (float *)dev_actual_residual,
			(cuComplex *) dev_orthogonal_basis);
	cudacheckSYN();

	set_alpha_beta_kernel<<<4, 1>>>((cuComplex *) dev_alpha,
			(cuComplex *) dev_beta);
	//don't synchronize
	...
	// fprintf(stderr, "h_residual_initial = %1.36f\th_res - tol = %1.36f\n", h_residual_initial, h_residual_initial-tolerance);
//============================================= END:residual_normalization_kernel ==================================================
	if (h_residual_initial > tolerance) {

		cudacall(
				cudaMemsetAsync((cuComplex *) dev_H_, 0,
						maxiter_plus_1 * maxiter * sizeof(cuComplex)));	//don't synchronize
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
		...
		if (for_gradient) {
			G_x_fft_matvec((cuComplex *) dev_gamma_array,
					(cuComplex *) dev_orthogonal_basis,
					(cuComplex *) dev_extended, (cufftHandle) plan, N);
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
		...
		cublascall(
				cublasCdotc((cublasHandle_t) *handle_p, N * N,
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
		...
		weight_subtract_kernel<<<gridsize, blocksize>>>(
				(cuComplex *) dev_orthogonal_basis + N * N,
				(cuComplex *) dev_H_, (cuComplex *) dev_orthogonal_basis);
		cudacheckSYN();
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
		...
		cublascall(
				cublasCsscal((cublasHandle_t) *handle_p, N * N,
						(const float *)dev_actual_residual + maxiter,
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
		...
		set_Identity_matrix_kernel<<<dim3(maxiter_plus_1, maxiter_plus_1),
				dim3(1, 1)>>>((cuComplex *) dev_Givens_rotation);
		cudacheckSYN();
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
		...
		set_first_Jtotal_kernel<<<maxiter_plus_1 * maxiter_plus_1 * 2, 1>>>(
				(cuComplex *) dev_Jtotal, (cuComplex *) dev_H_, maxiter,
				maxiter_plus_1);
		cudacheckSYN();
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
		...
		cudacall(
				cudaMemcpy(&h_Jtotal_0j, dev_Jtotal + maxiter_plus_1,
						sizeof(cuComplex), cudaMemcpyDeviceToHost));
		h_actual_residual = h_residual_initial
				* sqrt(
						(pow((float) h_Jtotal_0j.x, 2.0f)
								+ pow((float) h_Jtotal_0j.y, 2.f)));
		cudacall(
				cudaMemcpyAsync(dev_actual_residual + 1, &h_actual_residual,
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
			...
			for (unsigned int j = 0; j < GMRES_i + 1; j++) {
				cublascall(
						cublasCdotc((cublasHandle_t) *handle_p, N * N,
								(const cuComplex *) dev_orthogonal_basis
										+ j * N * N, 1,
								(const cuComplex *) dev_orthogonal_basis
										+ (GMRES_i + 1) * N * N, 1,
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
			...
			cublascall(
					cublasCgemm3m((cublasHandle_t) *handle_p, CUBLAS_OP_N,
							CUBLAS_OP_N, (unsigned int) GMRES_i + 1,
							(unsigned int) GMRES_i + 2,
							(unsigned int) GMRES_i + 2,
							(const cuComplex *) dev_alpha, (cuComplex *) dev_H_,
							(unsigned int) maxiter, (cuComplex *) dev_Jtotal,
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
			...
			//============================================= BEGIN: Create Givens_Rotation_Matrix ========================================
			set_4_Givens_rotation_matrix_elements_kernel<<<8, 1>>>(
					(cuComplex *) dev_Htemp, maxiter_plus_1,
					(cuComplex *) dev_Givens_rotation_0,
					(cuComplex *) dev_Givens_rotation_1,
					(cuComplex *) dev_Givens_rotation_2,
					(cuComplex *) dev_Givens_rotation_3, GMRES_i + 1);
			cudacheckSYN();
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
			...
			cublascall(
					cublasCgemm3m((cublasHandle_t) *handle_p, CUBLAS_OP_N,
							CUBLAS_OP_N, (unsigned int) GMRES_i + 2,
							(unsigned int) GMRES_i + 2,
							(unsigned int) GMRES_i + 2,
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
			...
			//===================================================== BEGIN: Update residual ======================================================
			cudacall(
					cudaMemcpy(&h_Jtotal_0j,
							dev_Jtotal + maxiter_plus_1 * (GMRES_i + 1),
							sizeof(cuComplex), cudaMemcpyDeviceToHost));
			h_actual_residual = h_residual_initial
					* sqrt(
							(pow((float) h_Jtotal_0j.x, 2.0f)
									+ pow((float) h_Jtotal_0j.y, 2.f)));
			cudacall(
					cudaMemcpyAsync(dev_actual_residual + GMRES_i + 1,
							&h_actual_residual, sizeof(float),
							cudaMemcpyHostToDevice));

			// fprintf(stderr, "h_actual_residual = %1.36f\n", h_actual_residual);
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
		...
		cublascall(
				cublasCgemm3m((cublasHandle_t) *handle_p, CUBLAS_OP_T,
						CUBLAS_OP_T, (unsigned int) GMRES_i,
						(unsigned int) GMRES_i, (unsigned int) GMRES_i + 1,
						(const cuComplex *) dev_alpha, (cuComplex *) dev_Jtotal,
						(unsigned int) maxiter_plus_1, (cuComplex *) dev_H_,
						(unsigned int) maxiter, (const cuComplex *) dev_beta,
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
		...
		set_cc_kernel<<<GMRES_i, 1>>>((cuComplex *) dev_cc,
				(cuComplex *) dev_Jtotal, (float *) dev_actual_residual,
				maxiter_plus_1);
		cudacheckSYN();
	...
	// h_computation_times_a[clock_i++] = std::chrono::high_resolution_clock::now();
		...
		if (GMRES_i > 0) {
			if (GMRES_i < 2) {
				...
				get_solution_kernel<<<gridsize, blocksize>>>(
						(cuComplex *) dev_solution, (cuComplex *) dev_cc,
						(cuComplex *) dev_orthogonal_basis);
				cudacheckSYN();
			} else {
				//============================================ BEGIN: Find solution to the LES(cc_new) for HH*cc_new=cc ============================================
				cuSolve_LES((cuComplex *) dev_HH, (cuComplex *) dev_cc, GMRES_i,
						cusolverH, (cuComplex *) dev_buffer_LES_cc,
						(int *) dev_Ipiv, (int *) dev_info,
...
void cuSolve_LES(cuComplex *dev_A, cuComplex *dev_B, const int m,
	...
	cusolvercall(
			cusolverDnCgetrf(cusolverH, m, m, dev_A, lda, dev_work, dev_Ipiv,
					dev_info));
	cudacheckSYN();
	...
	cusolvercall(cusolverDnCgetrs(cusolverH, CUBLAS_OP_N, m, 1, /* nrhs */
	dev_A, lda, dev_Ipiv, dev_B, ldb, dev_info));
	cudacheckSYN();
...
void show_norm(const char *description, cublasHandle_t handle,
	...
	cublascall(
			cublasGetPointerMode((cublasHandle_t) handle,
					(cublasPointerMode_t *) &mode_current));
	cublascall(
			cublasSetPointerMode((cublasHandle_t) handle,
					CUBLAS_POINTER_MODE_HOST));
	cublascall(
			cublasScnrm2((cublasHandle_t) handle, size_array,
					(const cuComplex *) dev_array, 1, (float *) &h_residual_0));
...
void show_norm_F(const char *description, cublasHandle_t handle,
	...
	cublascall(
			cublasSetPointerMode((cublasHandle_t) handle,
					CUBLAS_POINTER_MODE_HOST));
	cublascall(
			cublasSnrm2((cublasHandle_t) handle, size_array,
					(const float *) dev_array, 1, (float *) &h_residual_0));
