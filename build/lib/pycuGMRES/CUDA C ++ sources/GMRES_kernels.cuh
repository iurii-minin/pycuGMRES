						...
						const unsigned int N)
{
	...
	if ((i < size_limit) && (j < size_limit))
	{
		...
		float sigma = N * 0.6;
		Input_Field.x = Input_Field.x * exp(-pow((float)((float)(j) - N/2.f), 2.f)/pow(sigma, 2.f))/(sigma * sqrt(2.f * 3.14f));
		Input_Field.y = Input_Field.y * exp(-pow((float)((float)(j) - N/2.f), 2.f)/pow(sigma, 2.f))/(sigma * sqrt(2.f * 3.14f));
